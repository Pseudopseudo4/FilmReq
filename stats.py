...
class Stats(ctk.CTkFrame):
    ...
    def create_widgets(self):
        ...
        # Add button to switch to create_widgets2
        self.switch_button = ctk.CTkButton(
            self, text="Show More Stats", command=self.switch_to_widgets2
        )
    ...
    def create_widgets2(self):
        ...
        def process_user_files():
            # Determine the location of the userinfo folder
        ...
        # Process the user files to gather liked/disliked data
        ...
        # If liked categories exist, render the liked pie chart
        if liked_categories:
            ...
            ax_liked.set_title("Most Liked Categories")

            canvas_liked = FigureCanvasTkAgg(fig_liked, self.stat_frame)
            canvas_liked.draw()
            canvas_liked.get_tk_widget().pack()

        # If disliked categories exist, render the disliked pie chart
        if disliked_categories:
            ...
            ax_disliked.set_title("Most Disliked Categories")
        ...
        self.x_button = ctk.CTkButton(
            self, text="X", width=30, fg_color="red", hover_color="darkred",
            command=lambda: self.button_return()
        )
        self.x_button.pack(side="left", padx=(0, 5), pady=5)
        self.x_button.place(relx=1, rely=1, anchor='se')
